...
export default function InventarioPage() {
  ...
  // Lista única de categorías existentes (sin nulos ni vacíos)
  ...
  const hayItemsSinCategoria = items.some((it) => !it.categoria || !it.categoria.trim());
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <header className="flex items-start justify-between mb-6 gap-4 flex-wrap">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-300">
              {itemsFiltrados.length} de {items.length} {items.length === 1 ? "item" : "items"}
            </p>
          </div>
        ...
        {(categorias.length > 0 || hayItemsSinCategoria) && (
          <div className="flex flex-wrap gap-2 mb-6">
            <CategoriaChip
              label="Todas"
              activo={categoriaSeleccionada === "__todas__"}
              onClick={() => setCategoriaSeleccionada("__todas__")}
              count={items.length}
            />
            {categorias.map((cat) => (
              <CategoriaChip
                ...
                count={items.filter((it) => it.categoria === cat).length}
              />
            ))}
            {hayItemsSinCategoria && (
              <CategoriaChip
                label="Sin categoría"
                activo={categoriaSeleccionada === "__sin__"}
                onClick={() => setCategoriaSeleccionada("__sin__")}
                count={items.filter((it) => !it.categoria || !it.categoria.trim()).length}
              />
            )}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {itemsFiltrados.map((item) => (
              ...
              >
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-3 font-mono truncate">
                  {item.codigo_barras}
                </p>
              </Link>
            ))}
